...
https://github.com/madras-simulator/MADRaS/blob/master/MADRaS/utils/snakeoil3_gym.py

EDITED by AILab@FOI to connect to B.A.R.I.C.A. cloud server and use
...
query = '''
{
    "module":"TORCSMapModule",
    "method":"route",
    "args":[ "%s", "%s", %s ]
}
''' % ( "a", "g", '"type=highway", "landscape=farmland"' )
...
ophelp += ' --steps, -m <#>      Maximum simulation steps. 1 sec ~ 50 steps. \
            [100000]\n'
ophelp += ' --episodes, -e <#>   Maximum learning episodes. [1]\n'
ophelp += ' --track, -t <track>  Your name for this track. Used for learning. \
            [unknown]\n'
ophelp += ' --stage, -s <#>      0=warm up, 1=qualifying, 2=race, 3=unknown. \
            [3]\n'
...
def bargraph(x, mn, mx, w, c='X'):
    """Draw a simple asciiart bar graph.

    Very handy for visualizing what's going on with the data.
...
class Client(object):
    ...
    def respond_to_server(self):
        """Respond back to the TORCS server."""
        if not self.so:
            return
        try:
            message = repr(self.R)
        ...
        for k in sensors:
            if type(self.d.get(k)) is list:  # Handle list type data.
                ...
                #  strout=' -> '+strout[:9]+' '+strout[9]+' '+strout[10:]+' <-'
                    ...
                    strout += ' '.join(raw_tsens[:9]) + '_' + raw_tsens[9] \
                              + '_' + ' '.join(raw_tsens[10:])
                ...
                if k == 'gear':  # This is redundant now as it's part of RPM.
                    ...
                    strout = '%6.0f %s' % (self.d[k], bargraph(self.d[k], 0,
                                                               10000, 50, '~'))
                elif k == 'fuel':
                    strout = '%6.0f %s' % (self.d[k], bargraph(self.d[k], 0,
                                                               100, 50, 'f'))
                    ...
                    if self.d[k] < 0:
                        cx = 'R'
                    strout = '%6.1f %s' % (self.d[k], bargraph(self.d[k], -30,
                                                               300, 50, cx))
                elif k == 'speedY':  # This gets reversed for display .
                    strout = '%6.1f %s' %\
                        (self.d[k], bargraph(self.d[k] * -1, -25, 25, 50, 'Y'))
                elif k == 'speedZ':
                    strout = '%6.1f %s' %\
                        (self.d[k], bargraph(self.d[k], -13, 13, 50, 'Z'))
                elif k == 'z':
                    strout = '%6.3f %s' %\
                        (self.d[k], bargraph(self.d[k], .3, .5, 50, 'z'))
                elif k == 'trackPos':  # This gets reversed for display.
                    cx = '<'
                    if self.d[k] < 0:
                        cx = '>'
                    strout = '%6.3f %s' %\
                        (self.d[k], bargraph(self.d[k] * -1, -1, 1, 50, cx))
                elif k == 'stucktimer':
                    if self.d[k]:
                        strout = '%3d %s' %\
                            (self.d[k], bargraph(self.d[k], 0, 300, 50, "'"))
                    else:
                        strout = 'Not stuck!'
                elif k == 'rpm':
                    g = self.d['gear']
                    ...
                    if frontwheelradpersec:
                        skid = .5555555555 * self.d['speedX'] /\
                            frontwheelradpersec - .66124
                    strout = bargraph(skid, -.05, .4, 50, '*')
                elif k == 'slip':  # A sensible interpretation of wheel spin.
                    frontwheelradpersec = self.d['wheelSpinVel'][0]
                    slip = 0
                    if frontwheelradpersec:
                        slip = ((self.d['wheelSpinVel'][2] +
                                self.d['wheelSpinVel'][3]) -
                                (self.d['wheelSpinVel'][0] +
                                self.d['wheelSpinVel'][1]))
...
class DriverAction(object):
    ...
    (accel 1)(brake 0)(gear 1)(steer 0)(clutch 0)(focus -90 -45 0 45 90)
    (meta 0)
    """
    ...
    def clip_to_limits(self):
        ...
        if self.d['meta'] not in [0, 1]:
            self.d['meta'] = 0
        if type(self.d['focus']) is not list or\
...
def destringify(s):
    """Make a string into a value or a list of strings into a list."""
    if not s:
        return s
    if type(s) is str:
        try:
...
def drive_example(c):
    ...
    It will get around the track but the
...
# ================ MAIN ================
if __name__ == "__main__":
    BARICA_HOST = 'localhost'
    BARICA_PORT = 5000
    
    r = connect( BARICA_HOST, BARICA_PORT )
    print( "Connected to B.A.R.I.C.A.! Query results:" )
    print( r )
    print( "Starting TORCS simulation in 5 seconds..." )
    time.sleep( 5 )
    C = Client(p=3001)
