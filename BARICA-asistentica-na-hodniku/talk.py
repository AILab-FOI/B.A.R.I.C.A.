...
class FiniteStateMachine:
    ...
    def schedule_output( self ):
        ...
        global driver_for_shown_schedule
        try:
            DRIVER.execute_script( "alert('Dohvaćam podatke!');" )
        except WebDriverException:
            pass
        driver_for_shown_schedule = scrapSchedule( str( self.kind_of_study ), str( self.year_of_study ), str( self.group ) )
...
def start_presentation():
    dirname = os.path.dirname( os.path.abspath( __file__ ) )
    p = subprocess.Popen( [ 'hovercraft', os.path.join( dirname, 'presentation', 'prezentacija.rst' ), 'build' ], startupinfo=startupinfo )
    sleep( 3 )
    go_to_x_slide( '1' )
    press( 'f11' )
    hotkey( 'ctrl', '+' )
...
def start_text_to_speech():
    start_minimized( '/usr/bin/google-chrome', '--password-store=basic --window-position=0,0 --window-size=1,1 "https://voicenotebook.com/?autostart=1&chkbufer=1&pagelang=hr-HR"' )
    sleep( 1 )
...
if __name__ == '__main__':
    ...
    while True:
        ...
        try:
            ...
            print( 'Adding ponovi to buffer!' )
            print( e )
            logging.info( 'Adding ponovi to buffer!' )
            logging.info( e )
